...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	protected void connect(Connection con, NetworkInterface anotherInterface) {
		...
		if (this.host.getRouter().toString().contains("SprayAndWaitRouter2")) {			
			this.host.new_contact_between_nodes(anotherInterface.getHost());
			anotherInterface.getHost().new_contact_between_nodes(this.host);
		}
	}
