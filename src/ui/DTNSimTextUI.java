...
public class DTNSimTextUI extends DTNSimUI {
	...
	protected void runSim() {
		...
		this.calculateMaxAndMinConectivity();
		
		print("Simulation done in " + String.format("%.2f", duration) + "s");
	}
	
	public void calculateMaxAndMinConectivity() {
		int minConectivity;
		int maxConectivity;
		boolean haveMinConectivity, haveMaxConectivity;
		
		minConectivity = 0;
		maxConectivity = 0;
		haveMinConectivity = false;
		haveMaxConectivity = false;
		...
		while (it.hasNext()){
			...
			int min = host.getMinConectivity();
			int max = host.getMaxConectivity();
			
			if (!haveMinConectivity || min < minConectivity) {
				minConectivity = min;
				haveMinConectivity = true;
			}
			
			if (!haveMaxConectivity || max > maxConectivity) {
				maxConectivity = max;
				haveMaxConectivity = true;
			}			
		}
		
		System.out.println("Conectivity stats" + ", " + minConectivity + ", " + maxConectivity);
	}
